...
class Scheduler:
    ...
    def schedule(self) -> tuple[list[Sequence], bool]:
        ...
        while self.waiting and num_seqs < self.max_num_seqs: # bs必须小于max_num_seqs
            ...
            if num_batched_tokens + len(seq) > self.max_num_batched_tokens or not self.block_manager.can_allocate(seq): # 如果当前已经安排执行的序列数量加上这个序列的长度超过了最大批处理令牌数，或者块管理器无法为这个序列分配资源，那么就停止安排更多的序列执行。
                break
            num_seqs += 1
            self.block_manager.allocate(seq) # 块管理器为这个序列分配资源，准备将其安排执行。
        ...
        while self.running and num_seqs < self.max_num_seqs: # 只有self.running不为空(有decode阶段但是还没生成结束的任务)，并且当前已经安排执行的序列数量还没有达到最大批处理序列数时，才会继续安排更多的序列执行。
            ...
            while not self.block_manager.can_append(seq): # 如果不能够为这个序列追加资源了，那么就需要抢占正在运行的序列来腾出资源。抢占的方式是将正在运行的序列从running队列中弹出，并调用preempt方法将其状态设置为WAITING，并放回等待队列中。这个过程会一直进行，直到可以为这个序列追加资源了，或者没有更多的正在运行的序列可以被抢占了。
                if self.running:
                ...
                num_seqs += 1
                self.block_manager.may_append(seq) # 如果需要增加块，块管理器为这个序列追加资源，准备将其继续安排执行。
    ...
    def preempt(self, seq: Sequence): # preempt是抢占的意思，这里是把正在运行的seq抢占下来，放回等待队列中。这个方法会被调度器在调度过程中调用，当它需要腾出资源来安排新的序列时，就会调用preempt方法来抢占正在运行的序列。被抢占的序列会被设置为WAITING状态，并且被放回等待队列中，以便后续再次被调度器安排执行。
        ...
        self.block_manager.deallocate(seq) # 清除掉这个序列在块管理器中占用的资源，以便后续被其他序列使用。KVCache会被清空，块表会被重置，num_cached_tokens会被重置为0，但seq的tokens不会被修改，因为这些token是seq本地存储的，不依赖于块管理器的资源。
